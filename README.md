...
- **Резолв аккаунта для агента.** Инструменты агента требуют заголовок аккаунта,
  которого нет в JWT. Кабинет пока шлёт его сам — `X-Pharma-Account` из
  `GET /accounts/me`. Это доверие к клиенту и временный обход: заголовок уйдёт,
  когда агент начнёт резолвить аккаунт по `sub` —
  [SinopticsAI/pharma-agent#1](https://github.com/SinopticsAI/pharma-agent/issues/1).
- **CORS бакета `pharma-dossier`.** Документ уходит presigned `PUT` прямо в
  хранилище, минуя кабинет. Правило CORS ставится вручную прогоном в
  `pharma-edge`; без него браузер не отдаёт файл, и сообщение с вложением не
  уходит в нить. Бакет не наш, из этого репозитория правило не выставляется:

  ```powershell
  yc storage bucket update --name pharma-dossier `
    --cors "allowed-methods='[method-put,method-get,method-head]',allowed-origins='[https://pharma.sinoptics.ru]',allowed-headers='[*]',expose-headers='[ETag]',max-age-seconds=3600"
  ```

- **`X-Pharma-Locale` в `allowHeaders` шлюза.** Пока заголовка нет в списке
  разрешённых, preflight `POST /chat/{agentId}` отклоняется и диалог не идёт.
  Поэтому кабинет локаль заголовком не шлёт, и агент отвечает на `zh` —
  основном языке кабинета производителя. Выбор языка диалога вернётся, когда
  [`pharma_env`](https://github.com/SinopticsAI/pharma_env) добавит заголовок и
  перевыложит спецификацию шлюза.
