...
import { Callout, Card, DemoMark, Empty, KeyValue, NextAction, PageHeader, StatusBadge, Table } from '../kit'
...
export function CompaniesPage() {
  ...
  return (
    <Shell>
      <PageHeader title={t('nav.companies')} lead={t('home.lead')} />
...
export function CompanyRiskPage({ organizationId }: { organizationId: string }) {
  ...
  const card = companyCards(state).find((item) => item.organization.id === organizationId)
  ...
  return (
    <Shell>
      <PageHeader title={t('risk.title')} lead={t('risk.lead')} />
      <DemoMark>{t('shell.demoMark')}</DemoMark>
